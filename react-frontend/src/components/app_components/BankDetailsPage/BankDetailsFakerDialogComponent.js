...
import bankDetailsFakerFactory from "./bankDetailsFakerFactory";
...
const BankDetailsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = bankDetailsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("bankDetails").create(elem));
...
export default connect(mapState, mapDispatch)(BankDetailsFakerDialogComponent);
