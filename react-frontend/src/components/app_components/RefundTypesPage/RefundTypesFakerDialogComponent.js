...
import refundTypesFakerFactory from "./refundTypesFakerFactory";
...
const RefundTypesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = refundTypesFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("refundTypes").create(elem));
...
export default connect(mapState, mapDispatch)(RefundTypesFakerDialogComponent);
