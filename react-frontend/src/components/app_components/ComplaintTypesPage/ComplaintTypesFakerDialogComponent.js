...
import complaintTypesFakerFactory from "./complaintTypesFakerFactory";
...
const ComplaintTypesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = complaintTypesFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("complaintTypes").create(elem));
...
export default connect(mapState, mapDispatch)(ComplaintTypesFakerDialogComponent);
