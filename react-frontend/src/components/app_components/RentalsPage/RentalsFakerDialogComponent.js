...
import rentalsFakerFactory from "./rentalsFakerFactory";
...
const RentalsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = rentalsFakerFactory(props.user,fakerCount,customerNameIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("rentals").create(elem));
...
export default connect(mapState, mapDispatch)(RentalsFakerDialogComponent);
