...
import replacementsFakerFactory from "./replacementsFakerFactory";
...
const ReplacementsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = replacementsFakerFactory(props.user,fakerCount,customerNameIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("replacements").create(elem));
...
export default connect(mapState, mapDispatch)(ReplacementsFakerDialogComponent);
