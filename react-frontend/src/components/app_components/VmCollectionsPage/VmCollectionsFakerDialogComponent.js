...
import vmCollectionsFakerFactory from "./vmCollectionsFakerFactory";
...
const VmCollectionsFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [customerNameIds, setcustomerNameIds] = useState();
    ...
    useEffect(() => {
        ...
        client.service("customerDetails")
                        .find({ query: { $select: ["_id","customerName"] } })
                        .then((data) => {
                        setcustomerNameIds(data.data);
                        });
    ...
    const onRun = async () => {
        let fakeData = vmCollectionsFakerFactory(props.user,fakerCount,customerNameIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("vmCollections").create(elem));
...
export default connect(mapState, mapDispatch)(VmCollectionsFakerDialogComponent);
