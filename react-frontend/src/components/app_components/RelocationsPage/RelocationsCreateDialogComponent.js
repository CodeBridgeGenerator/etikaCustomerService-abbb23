...
const RelocationsCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            customerName: _entity?.customerName?._id,machineAddress: _entity?.machineAddress,relocationReason: _entity?.relocationReason,newOutlet: _entity?.newOutlet,
        ...
        const result = await client.service("relocations").create(_data);
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Relocations updated successfully" });
        props.onCreateResult(eagerResult.data[0]);
        } catch (error) {
            console.debug("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in Relocations" });
        }
        setLoading(false);
    };
    ...
    return (
        <Dialog header="Create Relocations" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="relocations-create-dialog-component">
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <InputText id="relocationReason" className="w-full mb-3 p-inputtext-sm" value={_entity?.relocationReason} onChange={(e) => setValByKey("relocationReason", e.target.value)}  />
            </span>
            <small className="p-error">
            {!_.isEmpty(error["relocationReason"]) ? (
              <p className="m-0" key="error-relocationReason">
                {error["relocationReason"]}
              </p>
            ) : null}
          </small>
            </div>
...
export default connect(mapState, mapDispatch)(RelocationsCreateDialogComponent);
