...
import relocationsFakerFactory from "./relocationsFakerFactory";
...
const RelocationsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = relocationsFakerFactory(props.user,fakerCount,customerNameIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("relocations").create(elem));
...
export default connect(mapState, mapDispatch)(RelocationsFakerDialogComponent);
