...
import agreementsFakerFactory from "./agreementsFakerFactory";
...
const AgreementsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = agreementsFakerFactory(props.user,fakerCount,customerNameIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("agreements").create(elem));
...
export default connect(mapState, mapDispatch)(AgreementsFakerDialogComponent);
