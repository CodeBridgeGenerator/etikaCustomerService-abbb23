...
import terminationsFakerFactory from "./terminationsFakerFactory";
...
const TerminationsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = terminationsFakerFactory(props.user,fakerCount,customerNameIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("terminations").create(elem));
...
export default connect(mapState, mapDispatch)(TerminationsFakerDialogComponent);
