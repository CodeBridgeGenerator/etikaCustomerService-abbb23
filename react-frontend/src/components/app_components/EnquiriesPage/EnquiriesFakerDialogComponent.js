...
import enquiriesFakerFactory from "./enquiriesFakerFactory";
...
const EnquiriesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = enquiriesFakerFactory(props.user,fakerCount,customerNameIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("enquiries").create(elem));
...
export default connect(mapState, mapDispatch)(EnquiriesFakerDialogComponent);
